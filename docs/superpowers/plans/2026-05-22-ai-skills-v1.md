...
**Architecture:** Keep v1 in Swift. A skill is a stored metadata record plus ordered workflow steps. Workflow execution is coordinated by `SkillRunner` and delegates side effects to injected adapters: permission decision provider, screenshot image provider, OCR provider, AI summary provider, email drafter, privacy access logger, and the custom automation process runner from `2026-05-22-command-palette-custom-automation-v1.md`. Do not create a second shell/Python execution layer. Tests use fakes and must not send email, run scripts, call AI/network APIs, or capture real screenshots.
...
This plan adds the first local AI Skills implementation surface. It does not add production network AI credentials, scheduled/background triggers, third-party integrations, cloud sync, or a visual node editor. Trigger metadata is stored and exposed in v1, but only manual command palette execution is active. `screenshotCaptured` and `clipboardChanged` are stored as planned trigger types only: UI must label them disabled/planned, command palette integration must not schedule or subscribe to them, and tests must prove they do not create background trigger registrations in v1.
...
- [x] **Step 3: Add the Swift module entry additively**
...
- [x] **Step 1: Add skill metadata and workflow model**
...
- [x] **Step 3: Verify model tests after project membership**
...
- [x] **Step 1: Add JSON-backed storage**
...
- [x] **Step 1: Add runner protocols and implementation**
...
The permission decision provider is the required Gate E boundary for v1. `SkillRunner.run(_:)` must ask it once before executing any step, and a denial must return before screenshot, automation, summary, or email draft providers are called. Wire the production provider to the existing app permission/feature decision surface if it exists; otherwise use an explicit conservative provider that denies missing OS/app permissions and can be replaced later.

The privacy access logger should adapt to Privacy Pulse when that plan has landed, using the shipped `PrivacyPulseAccessLogging` type/name. If Privacy Pulse is not present yet, keep `NoOpSkillPrivacyAccessLogger` as the default so AI Skills can compile without taking a hard dependency. Privacy events must include metadata such as access kind, skill id, and skill title only. Do not log screenshot bytes, OCR text, automation script contents/output, email recipients, subjects, draft body, prompts, or AI responses.

- [x] **Step 2: Add runner tests with fakes**
...
- [x] **Step 1: Add a skill command provider**
...
- [x] **Step 2: Add the palette destination additively**
...
- [x] **Step 3: Register the provider and panel builder**
...
struct AppSkillPermissionDecisionProvider: SkillPermissionDecisionProviding {
    var grantedPermissions: () -> Set<SkillPermission> = {
        // Conservative fallback until this is wired to the shipped app/OS permission registry.
        []
    }
...
enum SkillPrivacyAccessLoggerFactory {
    static func makeDefault() -> SkillPrivacyAccessLogging {
        // When Privacy Pulse is present, return its PrivacyPulseAccessLogging adapter here.
        NoOpSkillPrivacyAccessLogger()
    }
}
```

When Privacy Pulse types exist in the branch, add this adapter in `SkillRunner.swift` and return it from `SkillPrivacyAccessLoggerFactory.makeDefault()` using the shared app logger:

```swift
struct PrivacyPulseSkillAccessLogger: SkillPrivacyAccessLogging {
    let logger: PrivacyPulseAccessLogging

    func logAccess(kind: SkillPrivacyAccessKind, skillID: UUID, skillTitle: String) {
        logger.record(
            category: privacyCategory(for: kind),
            title: "AI Skill \(kind.rawValue)",
            detail: "Skill \(skillTitle) (\(skillID.uuidString))"
        )
    }

    private func privacyCategory(for kind: SkillPrivacyAccessKind) -> PrivacyPulseCategory {
        switch kind {
        case .screenCapture:
            return .screenRecording
        case .localAutomation, .emailDraft:
...
- [x] **Step 1: Enforce runner safety boundaries**

Confirm the implementation follows these rules:

- `SkillRunner` calls `SkillPermissionDecisionProviding` before the first workflow step and throws `SkillRunError.permissionDenied` without invoking screenshot, OCR, AI, automation, email, or privacy fakes when denied.
...
- Screenshot capture, local automation, and email draft creation log metadata through `SkillPrivacyAccessLogging`; when Privacy Pulse is present, adapt this to `PrivacyPulseAccessLogging.record(category:title:detail:)`, and when it is absent use `NoOpSkillPrivacyAccessLogger`.
- Privacy logging never records screenshot bytes, OCR text, automation scripts/output, email recipients, email subjects, draft bodies, prompts, or AI responses.
- `screenshotCaptured` and `clipboardChanged` remain disabled/planned in v1. Do not add file watchers, clipboard polling, screenshot observers, launch agents, timers, background subscriptions, or automatic scheduling for these trigger types.
...
- [x] **Step 2: Audit for forbidden side effects in tests**
...
- [x] **Step 3: Audit inactive trigger scheduling**

Run:

```bash
rg -n 'screenshotCaptured|clipboardChanged|Timer|NotificationCenter|NSPasteboard|schedule|observer|publisher|Task\\.detached' platforms/macos/Atlas/Skill*.swift platforms/macos/AtlasTests/Skill*.swift
```

Expected: matches for `screenshotCaptured` and `clipboardChanged` only occur in model serialization, UI planned/disabled labels, and tests proving they are inactive. There are no background schedulers, clipboard pollers, screenshot observers, detached tasks, or notification subscriptions for AI Skills triggers in v1.

## Task 8: Xcode Project Membership

- [x] **Step 1: Add new Swift files to project**
...
- [x] **Step 2: Verify project references**
...
- [x] **Step 2: Run command palette regression tests**
...
5. `SkillRunner` checks `requiredPermissions` through an injected permission decision provider before executing steps; denied runs call none of the side-effect providers.
6. Screenshot capture, local automation, and email draft creation emit Privacy Pulse-compatible metadata events when a logger is injected, with no sensitive payloads logged and a no-op fallback when Privacy Pulse has not landed.
7. Shell/Python steps reuse custom automation process runner concepts and do not instantiate a conflicting script execution layer.
8. Email workflow creates drafts only; tests never send real email.
9. Command palette returns skill commands only when the `skills` feature is enabled and does not schedule inactive background triggers.
10. Xcode project membership includes every new app and test Swift file.
11. Tests do not run real scripts, call network/AI APIs, send email, capture real screenshots, or install background trigger observers.
